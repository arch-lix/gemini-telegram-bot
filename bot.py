...
# Настройки
TELEGRAM_TOKEN = "8157269355:AAFOCDNdApPolAeBBjbY1An-OfYIokLvfKc"
...
def format_ai_response(text: str) -> str:
    ...
    # Обрабатываем \sqrt{число} -> корень(число)
    text = re.sub(r'\\sqrt\{([^}]+)\}', r'√(\1)', text)
    text = re.sub(r'\\sqrt\[([^]]+)\]\{([^}]+)\}', r'корень[\1](\2)', text)
    
    # Обрабатываем степени: ^{число} или ^число
    text = re.sub(r'\^\{([^}]+)\}', r'^(\1)', text)
    text = re.sub(r'\^([0-9])', r'^\1', text)
    
    # Обрабатываем индексы: _{число}
    ...
    # Убираем математические обертки $ полностью
    text = text.replace('$', '')
    
    # Выделяем заголовки (без жирного, просто добавляем отступ)
    text = re.sub(r'^### (.+)$', r'\n\1\n', text, flags=re.MULTILINE)
    text = re.sub(r'^## (.+)$', r'\n\1\n', text, flags=re.MULTILINE)
    text = re.sub(r'^# (.+)$', r'\n\1\n', text, flags=re.MULTILINE)
    
    # Убираем двойные звездочки (не используем жирный текст)
    text = re.sub(r'\*\*(.+?)\*\*', r'\1', text)
    text = text.replace('*', '')  # Убираем все оставшиеся звездочки
    ...
    # Убираем двойные звездочки (не используем жирный текст)
    text = re.sub(r'\*\*(.+?)\*\*', r'\1', text)
    text = re.sub(r'\*', '', text)  # Убираем все оставшиеся звездочки
    ...
    # Выделяем жирным текст в двойных звездочках
    text = re.sub(r'\*\*(.*?)\*\*', r'*\1*', text)
    
    # Убираем одиночные звездочки (которые могут сломать Markdown)
    # Заменяем их на обычный текст
    lines = text.split('\n')
    cleaned_lines = []
    for line in lines:
        # Считаем количество звездочек в строке
        star_count = line.count('*')
        # Если нечетное количество, убираем последнюю
        if star_count % 2 != 0:
            # Находим последнюю звездочку и заменяем на пустоту
            last_star_pos = line.rfind('*')
            if last_star_pos != -1:
                line = line[:last_star_pos] + line[last_star_pos+1:]
